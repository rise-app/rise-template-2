<script context="module">
  ...
  export async function preload({path, params, query}, {token, session_uuid, channel}) {
    ...
    const addressShippingReq = async () => {
      return rise.channelAuth.sessionCustomerShipping({}, {
    ...
    return Promise.all([
      ...
      .then(([
        ...
        return {
          addressShipping: addressShipping.data,
...
<script>
  ...
  async function setShipping(address) {
    inProgress = true
    return put(`auth/session/customer/address_shipping`, address, {}, $session.token, $session.session_uuid)
      .then(response => {
        inProgress = false
        ...
        if (response.data) {
          sessionValues.customer.address_shipping = response.data
        }
  ...
  onMount(async () => {
    return listCountryProvinces(addressShipping.country_code)
  })
...
<svelte:head>
  <title>Address Shipping - Account</title>
</svelte:head>
